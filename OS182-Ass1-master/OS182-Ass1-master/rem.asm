...
main(int argc, char const *argv[])
{
  ...
  15:	8d 4d d8             	lea    -0x28(%ebp),%ecx
	char arg1[MAX_VARIABLE_NAME];
	// char arg2[5];
	stringCopy(argv[1], arg1);
  18:	8b 58 04             	mov    0x4(%eax),%ebx
...
stringCopy(const char* s, char* t)
{
	int i=0;
	while(s[i]){
  1d:	0f b6 13             	movzbl (%ebx),%edx
  20:	84 d2                	test   %dl,%dl
  22:	74 12                	je     36 <main+0x36>
  24:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
		t[i]=s[i];
  28:	88 14 01             	mov    %dl,(%ecx,%eax,1)
		i++;
...
stringCopy(const char* s, char* t)
{
	int i=0;
	while(s[i]){
  2e:	0f b6 14 03          	movzbl (%ebx,%eax,1),%edx
  32:	84 d2                	test   %dl,%dl
  34:	75 f2                	jne    28 <main+0x28>
{
  ...
  39:	51                   	push   %ecx
  3a:	e8 93 03 00 00       	call   3d2 <remVariable>
	// remVariable(arg2);
	exit();
  3f:	e8 de 02 00 00       	call   322 <exit>
  44:	66 90                	xchg   %ax,%ax
  46:	66 90                	xchg   %ax,%ax
  48:	66 90                	xchg   %ax,%ax
  4a:	66 90                	xchg   %ax,%ax
...
stringCopy(const char* s, char* t)
{
  ...
  5f:	84 d2                	test   %dl,%dl
  61:	74 14                	je     77 <stringCopy+0x27>
  63:	90                   	nop
...
stringCopy(const char* s, char* t)
{
	int i=0;
	while(s[i]){
  6a:	0f b6 10             	movzbl (%eax),%edx
  6d:	83 c1 01             	add    $0x1,%ecx
  70:	83 c0 01             	add    $0x1,%eax
  73:	84 d2                	test   %dl,%dl
  75:	75 f1                	jne    68 <stringCopy+0x18>
		t[i]=s[i];
		i++;
	}
	return;
...
strncpy(char *s, const char *t, int n)
{
  ...
  c0:	eb 19                	jmp    db <strncpy+0x2b>
  c2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  c8:	83 c3 01             	add    $0x1,%ebx
  cb:	0f b6 4b ff          	movzbl -0x1(%ebx),%ecx
  cf:	83 c2 01             	add    $0x1,%edx
  d2:	84 c9                	test   %cl,%cl
  d4:	88 4a ff             	mov    %cl,-0x1(%edx)
  d7:	74 09                	je     e2 <strncpy+0x32>
  d9:	89 f1                	mov    %esi,%ecx
  db:	85 c9                	test   %ecx,%ecx
  dd:	8d 71 ff             	lea    -0x1(%ecx),%esi
  e0:	7f e6                	jg     c8 <strncpy+0x18>
    ;
  while(n-- > 0)
  e2:	31 c9                	xor    %ecx,%ecx
  e4:	85 f6                	test   %esi,%esi
  e6:	7e 17                	jle    ff <strncpy+0x4f>
  e8:	90                   	nop
  e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    *s++ = 0;
  f0:	c6 04 0a 00          	movb   $0x0,(%edx,%ecx,1)
  f4:	89 f3                	mov    %esi,%ebx
  f6:	83 c1 01             	add    $0x1,%ecx
  f9:	29 cb                	sub    %ecx,%ebx
  char *os;
...
000003c2 <setVariable>:
SYSCALL(setVariable)
 3c2:	b8 17 00 00 00       	mov    $0x17,%eax
 3c7:	cd 40                	int    $0x40
 3c9:	c3                   	ret    

000003ca <getVariable>:
SYSCALL(getVariable)
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 42f:	f7 f6                	div    %esi
 431:	0f b6 92 b8 07 00 00 	movzbl 0x7b8(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5a8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5ab:	b8 b0 07 00 00       	mov    $0x7b0,%eax
...
malloc(uint nbytes)
{
 ...
 746:	e8 e5 fe ff ff       	call   630 <free>
  return freep;
 74b:	8b 15 9c 0a 00 00    	mov    0xa9c,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 784:	eb e9                	jmp    76f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 786:	c7 05 9c 0a 00 00 a0 	movl   $0xaa0,0xa9c
 78d:	0a 00 00 
 790:	c7 05 a0 0a 00 00 a0 	movl   $0xaa0,0xaa0
 797:	0a 00 00 
    base.s.size = 0;
 79a:	b8 a0 0a 00 00       	mov    $0xaa0,%eax
 79f:	c7 05 a4 0a 00 00 00 	movl   $0x0,0xaa4
 7a6:	00 00 00 
