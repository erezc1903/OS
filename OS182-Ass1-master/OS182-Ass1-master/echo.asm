...
main(int argc, char *argv[])
{
  ...
  exit();
  5f:	e8 ae 02 00 00       	call   312 <exit>
  64:	66 90                	xchg   %ax,%ax
  66:	66 90                	xchg   %ax,%ax
  68:	66 90                	xchg   %ax,%ax
  6a:	66 90                	xchg   %ax,%ax
...
strncpy(char *s, const char *t, int n)
{
  ...
  b0:	eb 19                	jmp    cb <strncpy+0x2b>
  b2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  b8:	83 c3 01             	add    $0x1,%ebx
  bb:	0f b6 4b ff          	movzbl -0x1(%ebx),%ecx
  bf:	83 c2 01             	add    $0x1,%edx
  c2:	84 c9                	test   %cl,%cl
  c4:	88 4a ff             	mov    %cl,-0x1(%edx)
  c7:	74 09                	je     d2 <strncpy+0x32>
  c9:	89 f1                	mov    %esi,%ecx
  cb:	85 c9                	test   %ecx,%ecx
  cd:	8d 71 ff             	lea    -0x1(%ecx),%esi
  d0:	7f e6                	jg     b8 <strncpy+0x18>
    ;
  while(n-- > 0)
  d2:	31 c9                	xor    %ecx,%ecx
  d4:	85 f6                	test   %esi,%esi
  d6:	7e 17                	jle    ef <strncpy+0x4f>
  d8:	90                   	nop
  d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    *s++ = 0;
  e0:	c6 04 0a 00          	movb   $0x0,(%edx,%ecx,1)
  e4:	89 f3                	mov    %esi,%ebx
  e6:	83 c1 01             	add    $0x1,%ecx
  e9:	29 cb                	sub    %ecx,%ebx
  char *os;
...
putc(int fd, char c)
{
 ...
 598:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 59b:	b8 a9 07 00 00       	mov    $0x7a9,%eax
...
free(void *ap)
{
 ...
 69a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 69d:	a3 7c 0a 00 00       	mov    %eax,0xa7c
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 774:	eb e9                	jmp    75f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 776:	c7 05 7c 0a 00 00 80 	movl   $0xa80,0xa7c
 77d:	0a 00 00 
 780:	c7 05 80 0a 00 00 80 	movl   $0xa80,0xa80
 787:	0a 00 00 
    base.s.size = 0;
 78a:	b8 80 0a 00 00       	mov    $0xa80,%eax
 78f:	c7 05 84 0a 00 00 00 	movl   $0x0,0xa84
 796:	00 00 00 
